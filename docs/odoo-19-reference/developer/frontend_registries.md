# Frontend Registries — Fields, Views & Action Registries
...
Registries for extending the Odoo web client: field widgets, view types, action handlers, and service registries. Use when registering a custom field widget or view component.
...
:   Creates a new registry. Note that a registry is an event bus, so one can
    ...
    [`getAll`](#Registry.getAll "Registry.getAll") method returns a list of
...
| [main\_components] | top level components |
...
| [user\_menuitems] | menu items displayed in the user menu (top right of navbar) |

### Effect registry

The `effects` registry contains the implementations of all available effects.
See the section on the [effect service](services.html#frontend-services-effect-registry)
for more details.
...
[services](services.html#frontend-services) that should be activated by the Odoo
framework.
...
- `isDisplayed (optional)`: a function that takes the [env](framework_overview.html#frontend-framework-environment)
...
User menu items are defined by a function taking the [env](framework_overview.html#frontend-framework-environment)
