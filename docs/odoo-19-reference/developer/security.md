...
Controls who can read, write, create, and delete records. Covers access rights (ir.model.access), record rules, field-level access, and security groups. Use when implementing permissions for any model.
...
:   name
    ...
    group\_id
    :   The [`res.groups`](#res.groups "res.groups") to which the accesses are granted, an empty
        [`group_id`](#ir.model.access.group_id "ir.model.access.group_id") means the ACL is granted to *every user*
...
:   name
    ...
    groups
    :   The [`res.groups`](#res.groups "res.groups") to which access is granted (or not). Multiple
    ...
    global
    :   Computed on the basis of [`groups`](#ir.rule.groups "ir.rule.groups"), provides easy access to the
        global status (or not) of the rule.

    domain\_force
    :   A predicate specified as a [domain](orm.html#reference-orm-domains), the
        ...
        :   All the companies to which the current user has access as a list of
            ...
            [Security rules](../../howtos/company.html#howto-company-security) for more details.

    The `perm_{method}` have completely different semantics than for
    [`ir.model.access`](#ir.model.access "ir.model.access"): for rules, they specify which operation the rules
...
An ORM [`Field`](orm.html#odoo.fields.Field "odoo.fields.Field") can have a `groups` attribute
providing a list of groups (as a comma-separated string of
[external identifiers](../../glossary.html#term-external-identifiers)).
...
- restricted fields are removed from [`fields_get()`](orm.html#odoo.models.Model.fields_get "odoo.models.Model.fields_get")
  responses
...
Any public method can be executed via a [RPC call](../external_rpc_api.html#api-external-api-calling-methods) with the chosen parameters. The methods
...
Even better, there exists a [`SQL`](orm.html#odoo.tools.SQL "odoo.tools.SQL") wrapper to build queries
using templates that handles the formatting of inputs.
Check [SQL execution](orm.html#reference-orm-sql) for detailed usage.
...
not normalized.
Use `Domain` to handle safely the manipulation
of domains.
...
[`str`](https://docs.python.org/3/library/stdtypes.html#str "(in Python v3.13)") operations to *automatically escape parameters*.
