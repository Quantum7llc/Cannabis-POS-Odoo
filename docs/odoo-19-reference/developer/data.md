...
Odoo modules ship data as XML or CSV files loaded at install/upgrade time. Covers records, references, functions, and demo data. Use when defining default records, configuration data, or demo datasets.
...
`id`
:   the [external identifier](../../glossary.html#term-external-identifier) for this record. It is strongly
...
`search`
:   for [relational fields](orm.html#reference-fields-relational), should be
    a [domain](orm.html#reference-orm-domains) on the field’s model.
    ...
    the field is a [`Many2one`](orm.html#odoo.fields.Many2one "odoo.fields.Many2one")

`ref`
:   if a `ref` attribute is provided, its value must be a valid
    [external id](../../glossary.html#term-external-id), which will be looked up and set as the field’s value.

    Mostly for [`Many2one`](orm.html#odoo.fields.Many2one "odoo.fields.Many2one") and
    [`Reference`](orm.html#odoo.fields.Reference "odoo.fields.Reference") fields
...
:   if a `type` attribute is provided, it is used to interpret and convert
    ...
    :   extracts the `field`’s children as a single document, evaluates
        any [external id](../../glossary.html#term-external-id) specified with the form `%(external_id)s`.
...
:   for cases where the previous methods are unsuitable, the `eval`
    ...
    identifiers](../../glossary.html#term-external-identifiers) (`ref`) and the model object for the current field if
    applicable (`obj`)
...
`parent`
:   - If a `parent` attribute is set, it should be the [external id](../../glossary.html#term-external-id)
...
:   A `groups` attribute is interpreted as a comma-separated sequence of
    [external identifiers](../../glossary.html#term-external-identifiers) for `res.groups` models. If an
    [external identifier](../../glossary.html#term-external-identifier) is prefixed with a minus (`-`), the group
    is *removed* from the menu’s groups

`action`
:   if specified, the `action` attribute should be the [external id](../../glossary.html#term-external-id)
...
Creates a [QWeb view](../user_interface/view_architectures.html#reference-view-architectures-qweb) requiring only the `arch`
...
:   same as the corresponding field on `ir.ui.view` (nb: `inherit_id`
    should be an [external identifier](../../glossary.html#term-external-identifier))
...
[access rights](security.html#reference-security-acl):

- the file name is `model_name.csv`
- the first row lists the fields to write, with the special field `id`
  for [external identifiers](../../glossary.html#term-external-identifiers) (used for creation or update)
...
- the first column is the [external id](../../glossary.html#term-external-id) of the record to create or
  update
- the second column is the [external id](../../glossary.html#term-external-id) of the country object to link
