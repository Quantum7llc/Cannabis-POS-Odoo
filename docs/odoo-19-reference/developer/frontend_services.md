# Frontend Services — RPC, Notifications, Dialog & More
...
Built-in Odoo frontend services: RPC, ORM, notifications, dialogs, router, user session, and more. Use when calling backend methods or showing UI feedback from JavaScript.
...
:   Arguments
    ...
    `hashchange` event, nor a `ROUTE_CHANGE` in the [main bus](framework_overview.html#frontend-framework-bus).
...
> **Note:**
>
> The router service emits a `ROUTE_CHANGE` event on the [main bus](framework_overview.html#frontend-framework-bus)
...
- an event `RPC_ERROR` is triggered on the main application bus. The event payload
  ...
  `{type: 'network'}`.
  When a network error occurs, a [notification] is
...
> **Note:**
>
> The scroller service emits a `SCROLLER:ANCHOR_LINK_CLICKED` event on the [main bus](framework_overview.html#frontend-framework-bus).
...
hasGroup(*group*)
:   Arguments
    :   - **group** (`string()`) – the xml\_id of the group to look for
...
- Depending on your settings, the server may not regenerate the assets bundles
  ...
  - in debug mode, there is an option in the debug menu ( button in the navbar)
...
- The *display\_notification* client action allows to trigger the display
...
cheap to compute (a slow session\_info call will delay the loading for the web
...
:   This is the default field type for fields of type `datetime`. The values are always
    ...
    field, see [Date Field] description.
...
:   > The `domain` field allows the user to construct a technical-prefix domain
    ...
    > - `count_limit` (default: `10000`): the domain widget performs search\_count requests to validate
    >   the domain, and to indicate the number of records matching it. This requests might be costly
    >   on large tables, so by default, the search\_count is limited to 10000.
...
:   Displays the value inside a bootstrap badge pill.
    ...
    by using the [Decoration] mechanism.
...
- from the perspective of the server, it is a record of the model *ir\_action*,
