# Actions — Window, Server, Client & URL Actions
...
Actions control what happens when a user clicks a menu item or button: window actions open views, server actions run Python, client actions trigger JavaScript. Use when wiring up menus, buttons, or automated workflows.
...
- A string
  :   if a [client action] matches, interpret as
...
[views](../user_interface/view_records.html): a window action defines a set of view types
...
`view_ids`
:   M2M[1] to view objects, defines the initial content of
    `views`
...
*class* odoo.addons.base.models.ir\_actions.IrActionsServer(*env: Environment*, *ids: [tuple](https://docs.python.org/3/library/stdtypes.html#tuple "(in Python v3.13)")[IdType, ...]*, *prefetch\_ids: Reversible[IdType]*)[[source]](https://github.com/odoo/odoo/blob/19.0/odoo/addons/base/models/ir_actions.py#L566)
...
:   fields to override when creating or copying the record.
    [`One2many`](orm.html#odoo.fields.One2many "odoo.fields.One2many") with the fields:
...
also need to specify `binding_model_id` from [Bindings]. It’s
...
`groups_id`
:   [`Many2many`](orm.html#odoo.fields.Many2many "odoo.fields.Many2many") field to the groups allowed to view/use
    the current report
...
`paperformat_id`
:   [`Many2one`](orm.html#odoo.fields.Many2one "odoo.fields.Many2one") field to the paper format you wish to
...
by calling [`IrCron._commit_progress()`](#odoo.addons.base.models.ir_cron.IrCron._commit_progress "odoo.addons.base.models.ir_cron.IrCron._commit_progress") and checking the result. The progress
...
IrCron.method\_direct\_trigger()[[source]](https://github.com/odoo/odoo/blob/19.0/odoo/addons/base/models/ir_cron.py#L150)
...
IrCron.\_trigger(*at: datetime | Iterable[datetime] | [None](https://docs.python.org/3/library/constants.html#None "(in Python v3.13)") = None*)[[source]](https://github.com/odoo/odoo/blob/19.0/odoo/addons/base/models/ir_cron.py#L735)
:   Schedule a cron job to be executed soon independently of its
    ...
    datetime. The actual implementation is in `_trigger_list()`,
    which is the recommended method for overrides.

    Parameters
    :   **at** – When to execute the cron, at one or several moments in time
...
[`IrCron.method_direct_trigger()`](#odoo.addons.base.models.ir_cron.IrCron.method_direct_trigger "odoo.addons.base.models.ir_cron.IrCron.method_direct_trigger") in the registry test mode.
