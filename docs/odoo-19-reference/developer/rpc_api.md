# External API — XML-RPC, JSON-RPC & Python Client
...
Odoo's external API over XML-RPC and JSON-RPC. Covers authentication, model method calls, search/read patterns, and the official Python client library. Use when integrating external systems with Odoo.
...
[Models](backend/orm.html#reference-orm-model) API is easily available over HTTP via the `/json/2` endpoint.
...
ids
:   An array of record ids on which to execute the method. Empty or omitted when calling an
...
Create a new API key for a user via Preferences ‣ Account Security ‣
New API Key.
...
The Generate Key button creates a strong 160-bits random key. The key value is displayed
...
Please refer to [OWASP’s Secrets Management Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/Secrets_Management_Cheat_Sheet.html#secrets-management-cheat-sheet)
for further guidance on the management of API keys.

### Access Rights

The JSON-2 API uses the standard [security models of Odoo](backend/security.html#reference-security). All operations
...
- The [Access Log fields](backend/orm.html#reference-fields-automatic-log-access) use the bot account. No user is impersonalized.
...
hosts multiple databases and the [dbfilter](../../administration/on_premise/deploy.html#dbfilter) wasn’t configured to use the `Host` header.
...
> **Note:**
>
> - [Tutorial to create a module](../tutorials/server_framework_101.html)
> - PostgreSQL - Transaction Isolation - [Repeatable Read](https://www.postgresql.org/docs/current/transaction-iso.html#XACT-REPEATABLE-READ)

## Code Example

The following examples showcase how to execute two of the [common ORM methods](backend/orm.html#reference-orm-models-crud) on a dummy database `mycompany` hosted on the dummy website
`https://mycompany.example.com`. Its [dynamic documentation] would be available at <https://mycompany.example.com/doc>.
...
> **Note:**
>
> [Database Manager Security](../../administration/on_premise/deploy.html#db-manager-security)
...
1. `/web/database/create` takes inputs `master_pwd`, `name`, `login`, `password`,
   `demo`, `lang`, and `phone`.
2. `/web/database/duplicate` takes inputs `master_pwd`, `name`, `new_name`, and
   `neutralize_database` (not neutralized by default).
3. `/web/database/drop` takes inputs `master_pwd` and `name`.
4. `/web/database/backup` takes inputs `master_pwd`, `name`, and `backup_format` (zip by
